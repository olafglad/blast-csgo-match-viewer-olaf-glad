...
import type {MatchData} from "../../shared/index.js";
...
try {
  ...
  console.log(
    `Teams: ${matchData.teams[0].name} vs ${matchData.teams[1].name}`
  );
  console.log(
    `Final: ${matchData.teams[0].finalScore} - ${matchData.teams[1].finalScore}`
  );
} catch (error) {
